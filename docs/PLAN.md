...
| 19 — the checker can see a row that never reached the sheet | done | `check.js` looked for symbol geometry, and a coupler has none — it is a link row, and link rows were taken out of the set `missing` is computed over, so `check.items` said 5/5 drawn on the nine-row `warnings/coupler_invalid` where two rows were absent from the SVG entirely. A link row must still own a `<g>` in `scene.groups`; that alone found **five** cases, not the one expected, because a *valid* changeover had no group either (its label is drawn inside its generator's). `expectedEdges` stopped keeping a third definition of what a coupler ties, `fixtures.test.js` stopped skipping couplers, and W1's `orphansStillDrawn` — which asked whether the *deleted* board was still drawn, always no — now asks about the orphans, which were drawing correctly all along |
| 20 — a coupler draws, or it says why | done | What a coupler is now lives once in `src/core/couplers.js` (beside `supplies.js`, not under `rules/`: the page draws without running the rules). The rule records it, the drawing draws it, the checker expects it. They had each re-derived it and disagreed — on `RMU1, RMU2` the rule said invalid, the drawing skipped the row and the checker expected a conductor; on `BB1, BB2, TX1` the rule said invalid and the drawing drew an ordinary tie with **no message at all**, discarding `TX1`. A coupler that cannot be a tie now runs from the busbar it has into the lane above the bar row and ends open, stacking when two share a board; with no placed busbar it stands in the leftover column. `couplerDiagnostics` replaced the renderer writing English that `diagnostics.js` matched with a regex to recover the code, and `render()` no longer takes a warnings array. New `COUPLER_EXTRA_SUPPLY`; new `warnings/coupler_open_ends`. T6 is still 2 actions (a chip drop and a Shift-drag — the drop is an action too, so the plan's "2 → 1" was wrong) but no ID is typed: `typoProof` false → true |
| 21 — a supply named `constructor` no longer kills the drawing | done | `buildModel` keyed rows on `{}`, so `constructor`, `toString`, `valueOf`, `hasOwnProperty` and `__proto__` found an inherited function, were taken for rows that exist, and threw `items[q].parents is not iterable` — no drawing, no diagnostic, no export, nothing on screen naming the cell. `Object.create(null)`, and a row may be called one. Baseline W8 |
| 22 — one rule for what an ID is | done | Fourteen places decided whether two names were one ID; twelve by exact string, and the two that were not were exactly the two that talk to the surveyor. So `tx1` beside a `TX1` row drew a red "unknown ID — did you mean TX1?" while `nextId` refused to hand out `TX1` because it thought `tx1` held it, and naming a second row `tx1` was accepted in silence. `idKey` in `types.js` is that rule now. Feeds From matches exactly first and falls back to it: `bb1` and `BB 1` are `BB1`, connected, with `SUPPLY_CASE` saying how it was read — `supplies` still keeps what was written. **`DUP_ID` stays exact**, so no legal sheet becomes illegal; `ID_CASE_CLASH` names two rows that differ only that way and says references go to the first. `renameReferences` carries a reference however spelled; `canFollowRename` refuses a rename onto a name that would be ambiguous. Baseline W2: an error with a suggestion becomes no error |
| 23 — a column binds by its name, and the delimiter is sniffed | done | `col("id")` was `h.includes("id")`, first hit wins, so a `Building ID` column left of `ID` bound every row's identity to the building number in silence. Detection was exact while binding was loose — the worst pairing: `Equipment ID \| Equipment Type` was refused outright while the decoy was accepted and misread. Exact names bind first, the substring pass is the fallback, one column serves at most one field, detection gained the same fallback, and `readTable` reports a loose binding at import. The delimiter is sniffed from comma, semicolon and tab (a European-locale export used to die with "no header row"). `src/io/csv.js` is the only reader — the page's hand-synced copy is gone and csv.js joined the bundle; the workbook reader no longer demands a sheet named `Equipment`; a zero-row import leaves the table alone; the CLI prints one line instead of a stack trace. New `test/io.test.js`; baseline T7, W7 |
| 24 — the feeder is a placeholder | done | Attach anything to a feeder and its own breaker duplicated the equipment's; a non-board child was orphaned into the leftover column past the end of the bar, hung from `Y_BUS` in mid-air, and the edge read disconnected. Four changes. **The device is what the row names**: `propose.js` no longer fills `"Feeder": "CB"` and `render.js` no longer invents one, so an empty Protection cell draws a bare conductor and a filled one draws — keyed on the raw cell, not on the canonical kind, so a written-but-unrecognised `Thermal relay` still draws its default glyph as `UNKNOWN_PROT` promises. **Anything may hang there**: `supplies.js` dropped the rank-0 line that refused a feeder everything but a board and an MCC (a constraint removed, nothing newly refused — a pump on a way scores 1, still below the board in the picker), and `MCC_BAD_SUPPLY` accepts a feeder. **The way is a link**: `layout.js` gained `feederBoard` and `carriesOn` — one judgement read by the layout, the drawing and the checker (§5) — so the way's slot widens for what hangs on it, the equipment is placed at the way's `x` and starts at the foot of the way's own device, the arrow goes (the way is no longer an open end) and the checker judges the row by its group and reads the edge as board ~ load. **The devices answer for a row**: the two breakers on a sub-board run were drawn outside any group — on the sheet but belonging to nobody, unclickable and invisible to the checker's ownership map; each half is now drawn in its own row's group (13 goldens regroup, geometry identical), and `scene.js` records a device's layer so the legend's are the only ones left unowned. New `testdata/topics/feeder_placeholder` and `test/feeder.test.js`; baseline T8 (drop a way, drop a motor on it: 2 actions, one device on the run). `audit/w08_wrongloads` moves deliberately, 5 disconnected → 3. **The one cost, and it reaches user files**: a sheet that left Protection blank on ways that do have breakers loses them from the drawing. The cell is visibly empty and the equipment table prints it empty, so the drawing and the table agree; said in the README and the page's how-to. Not done: a feeder with two supplies still draws on `parents[0]` only, and a feeder whose Description says *capacitor* / *NER* / *arrester* is still that item rather than a placeholder |
| 26 — the drop-downs offer what there is | done | The Protection cell offered six words from a hand-written `<datalist>` in the page's HTML — the same six for a ring main unit and for a motor, and **without MCCB, MCB, ACB or VCB**, words the reader has always understood and a surveyor writes constantly. Voltage and Rating had followed the row's Type since phase 1; Protection never had. `src/core/protection.js` is that list now, in the shape of `supplies.js`: `PROT_LABELS` (every device worth writing, with what it is) and `protCandidates(type, variant)` — the whole vocabulary every time, the gear that Type usually carries first, the rest marked *unusual here*, nothing hidden. An RMU is offered `LBS`, a motor `Contactor`, a way `MCCB`, a transformer `Fuse-switch`, a UPS `MCCB` (a symbol variant is wired like its family and protected like its own gear). The rule the tests hold: **the page must never offer a word the reader will then report as unknown**. Two additions to `PROT_ALIASES` follow from that — `RCBO`/`RCCB`/`RCD`/`ELCB` are breakers (the earth-leakage element has no symbol of its own), and `Unknown`/`TBC` is *a device is there, its type was not recorded*: the default symbol, and no complaint, because it is a deliberate answer rather than a typo. `Thermal relay` stays unknown and stays reported — it is a relay, not a switching device. `sites/c2_building` writes `RCBO` and had been warning about it since the pool was built; its drawing is byte-identical and the warning is gone. The Feeds From picker already listed every ID ordered by `supplyCandidates`; the one gap was a row with no Type yet, which fell back to a wall of bare IDs — it now says what each item is, so the list is a search of the equipment. `test/protection.test.js` and two smoke checks |
| 27 — a way is a way, on either side of the transformer | done | From testing: attach a transformer to a feeder on an **MV** busbar and it drew floating with "supply not defined", while the table plainly named its supply. Phase 25 had scoped the placeholder to LV boards and MCCs — `feederBoard` returned `null` for a way on MV gear, so `carriesOn` reported it carried nothing and every rule built on that judgement skipped the MV side. Measured on four sheets, **every** child type on an MV way was disconnected and `warnings` was empty in all four: a transformer landed 600 px past the board dragging its own LV board with it, an MCC drew on the LV bar row, and on an RMU the transformer landed to the *left* of its own switchgear. Four changes. **One judgement, widened once**: `feederBoard` accepts `MV_BUSBAR` and `RMU` (LV types first, so a row naming two supplies resolves as before), and the two places that had copied it inline — `expectedEdges` and `carriedBy` — call it instead; three opinions on what a way is would have been exactly the drift §5 exists to prevent. **The slot**: `slotWidth` makes a carrying way as wide as what it carries and `placeCarried` lays that across the way's own slot, the MV half of what `lvKidWidth`/`placeLvBoard` already did — both guarded on `carriesOn(...).length`, so 79 of 80 goldens were byte-identical. **The drawing**: the MV branch stops terminating when the way carries something (the arrowhead had been landing *on* the transformer, with the run drawn twice down one x); the transformer loop gained the `FEEDER` branch it never had, drawing the transformer's own device below the way's — two cells, two devices, the rule the user chose over an MV-only special case; the open end survives only where a way genuinely cannot be followed, because there the note is the truth; and an RMU hands over a **bare** conductor from the bottom of its box, since it draws every way's device inside the enclosure and a second would be the same switch twice. A motor was also being moved to the LV feeder band merely for being named on a way — a way does not move a motor to another row. The RMU-without-a-switchboard sheet reaches placement from the opposite end (the transformer follows its board, the way is left behind), so rather than a third placement rule `shareWayColumn` reconciles them: a way and what it carries stand in one column, whichever was placed first. **The silence**: new `WAY_NOT_ON_BOARD` — a "way" that is not on a board (named on a motor, or with an unresolved supply) has nothing to hang anything from, and the row it carries used to draw floating with no message naming it (§6). New `testdata/topics/mv_way_placeholder` and `topics/rmu_way_placeholder`; `test/feeder.test.js` gained the MV shapes and the §6 guard; baseline T9. `warnings/mcc_on_mv` moves and is now clean rather than pinning its own broken drawing |
| 28 — the proposal audit | done | A third instrument beside the goldens and the entry baseline: **is what the engine proposes what the sheet wanted?** `tools/proposal-audit.mjs` generates plausible sites from a seeded grammar (`tools/lib/configs.mjs`, 16–30 rows, twelve shapes, two row orders — network order as the fixtures are written, survey order as a person walks a switchroom), scores each `P = A × C × V` (shape frequency in the field-like fixtures × hard consistency checks × how often the pool writes each value; a site with an absurd row is rejected, not merely marked down), builds each in the engine the way a surveyor does — drop the item on its supply, read the proposal, correct it, fill the rest — and ranks the corrections by `breadth × depth × mean P`. Every correction is labelled against the pool (`tools/lib/pool.mjs`: `sites/`, `audit/`, `examples/`, one merged sheet excluded, per-fixture weights, blanks never evidence) as POOL / MIXED / OPINION / BLANK, and only POOL makes the headline — the generator's opinion is kept apart from what real sheets say, and POOL needs three rows on two sheets. A page pass (`--page`) rebuilds one sheet per shape through `addRowFor` and found **0 mismatches in 247 drops** between what the page hands the surveyor and what the engine returns, and reads where each intended value sits in the real pickers. The first report (`testdata/proposals/AUDIT.md`, seed 2026, 188 plausible sites of 200): the pool writes a device on 98 % of ways, so the placeholder decision of phase 25 is the #1 correction by potential and is reported as the deliberate trade it is, with a way out (propose it, clear it when something is dropped on the way while the cell is still the engine's); a motor in an MCC is written `Fuse-contactor` where `Contactor` is proposed; a coupler's second end and a ring's other side are never proposed; an RMU's `LBS, LBS` can never be, because `propose.js:152` needs the comma at drop time; a palette click lands a motor on the board when the pool puts motors on MCCs 7.8× as often; a transformer on a way is read as LV gear whatever board the way leaves; the quick lists lack the MCC ratings the pool actually writes (400/250/160 A), 30/45/15 kW motors, 32 A feeders and MV voltages for MV ways and ties. The audit **proposes; it does not change the engine** — each class ends with the change that would remove it, sized. `test/proposal-audit.test.js` pins the pool's exclusions, the checks (they reject `audit/w08_wrongloads` and accept `sites/c3_pumps`), determinism, and the classifier's rules |
| 29 — the audit's first round of fixes | done | Four proposals the field sheets disagreed with, all found by the audit (row 28): a motor dropped on an MCC proposed `Contactor` where every filled cell in the pool said fused contactor — `proposeProt` now says `Fused contactor` on an MCC, the word the symbol registry and the pool spell it with, offered first for a motor; a palette click put a new pump on the LV board when the pool has motors on an MCC 7.8× as often — `USUAL_SUPPLIES[PUMP]` leads with the MCC; a transformer dropped on a way out of MV gear got no ratio and, on an RMU's way, a breaker for a fuse-switch, because `proposeRow` read the feeder as LV gear — it resolves the way to its board (`feederBoard`) for the label and the device, the motor alone keeping its starter; and a coupler never got its far end — `proposeOtherEnd` names it when exactly one board of the kind or one genset can be it, sheet order kept, and the page re-proposes the device and the label when a supply changes under a still-tinted cell (`reproposeFor`), so an RMU Shift-dragged onto its second link reads `LBS, LBS`. The rating lists gained the values the audit saw typed (32 A on a way, 160/250/400 A on a board, 15/30/45/355 kW and 1.5 MW on a motor, 10 MVA, 1250 kVA). Not changed: the blank feeder, a design choice recorded as the audit's #1 finding; a ring's other link, which is the surveyor's call. |
