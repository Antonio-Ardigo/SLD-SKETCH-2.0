...
| 14 — Enter repeats the row above | planned | `addRow()` proposes the row above's Type with its supply as the target, so a run of feeders or pumps arrives typed, numbered, with device and voltage; `+ Add row` keeps the blank row. T1: 15 → 10. Removes the typeless-row-with-inherited-supply risk (W3) |
| 15 — paste from a spreadsheet | planned | a `paste` listener on the table: tab / `;` / `,` blocks with or without a header row, appended at the paste point; the page's duplicate `parseCsv`/`tableToRows` retired for `src/io/csv.js`, which learns the two delimiters. T2: 5 → 2, table kept |
| 16 — renaming an ID renames its references | planned | on commit of the ID cell, every `Feeds From` token equal to the old ID is rewritten, one undo step, graph signature identical (constitution §3); a rename onto an existing ID is refused. T3: 6 → 1, no stale drawing |
| 17 — draw through errors, and name the fix | planned | `draw()` and `redraw()` stop withholding the picture on `UNKNOWN_SUPPLY` / `DUP_ID`: the row draws floating with its message (constitution §6), exports are allowed; an unknown supply gets a suggestion — case-insensitive match, then edit distance ≤ 2 — as a named fact, with a one-click **use BB1** in the problems box. Fixtures `warnings/unknown_supply_draws`, `warnings/dup_id_draws`. T4: 2 → 1 with the drawing on screen; W1, W2 stop being stale |
| 18 — re-wire by dragging the symbol | planned | a drag that starts on a symbol re-parents it: release on a board / RMU / transformer / MCC writes that row's `Feeds From`; Shift appends a second supply; a rank-0 target is refused. Panning from empty canvas is unchanged. T5: 3 → 1; T6 typo-proof |
